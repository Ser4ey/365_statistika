...
import info
import os
LOG_FILENAME = 'vilki_logs.csv'



class Work_With_My_Logs:

    def __init__(self):
        '''
        Формируем массив из логов
        Игнорируем пустые строки и 1 строку

        Готовый массив от новых к старым
        '''
        self.all_data = []
        with open(LOG_FILENAME, encoding='utf-8') as r_file:
            ...
            for row in file_reader:
                if len(row) < 12:
                    continue
                if row[0] == 'вид спорта':
                    print('Пропущена 1 строка')
                    continue

                self.all_data.append(row)
                i += 1

            print(f'Было полученно {i} ставок из логов')

            # Переворачиваем логи - от новых к старым
            self.all_data = self.all_data[::-1]


    def get_info(self, team_name):
        '''Сомодостаточная функция
        Принемает имя общее имя команд из bet365 и возвращает информацию из логов
        Обрезает список удоляя старые команды
        '''

        for i in range(len(self.all_data)):
            bet_data = self.all_data[i]
            team1 = bet_data[4]
            team2 = bet_data[5]
            if team1 in team_name and team2 in team_name:
                self.all_data = self.all_data[i+1:]
                print(bet_data)
                return bet_data

        print('Не удалось найти ставку')
...
class FireFoxDriverWithProxy:
    def __init__(self, path_to_geckodriver, user_agent, proxy, proxy_login_and_password, type_of_account, final_balance, account_code_name, is_reversed):
        ...
        driver = webdriver.Firefox(capabilities=firefox_capabilities, firefox_binary=info.firefox_binary,
...
class FireFoxDriverWithVPN(FireFoxDriverWithProxy):
    def __init__(self, type_of_account='.com', final_balance='0',
                 account_code_name='No', is_reversed='False'):
        self.is_VPN = True
        firefox_capabilities = webdriver.DesiredCapabilities.FIREFOX
        firefox_capabilities['marionette'] = True

        # путь к firefox аккаунту !!!
        fp = webdriver.FirefoxProfile(info.path_to_firefox_profile)
